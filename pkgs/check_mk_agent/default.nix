{
  ...
  localChecks ? [],
  cmkaVersion ? "v2.4.0p24",
  cmkaSrc ?
    fetchFromGitHub {
      owner = "Checkmk";
      repo = "checkmk";
      rev = cmkaVersion;
      sha256 = "sha256-28pJHfBMcK26Kgq9l+ACtXtaUW6H9+rwc0nXyV3Ml1k=";
    },
}: let
  deps = with pkgs; [
...
in
  stdenv.mkDerivation {
    ...
    installPhase =
      ...
      ''
      + (
        if (lib.length pluginsToInstall) != 0
        then "patchShebangs $out/usr/lib/check_mk_agent/"
        else ""
      );
  }
